 ...
 */
export async function generateEmbedding_2(text: string, retries: number = 3, timeoutMs: number = 10000): Promise<number[]> {
  ...
  for (let i = 0; i < texts.length; i += batchSize) {
    ...
    const batchEmbeddings = await Promise.all(
      batch.map(async text => {
        try {
          return await generateEmbedding_2(text);
        } catch  {
...
export const aiSummariseCommit_2 = async (diff: string): Promise<string> => {
  try {
